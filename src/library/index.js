...
export var analyzeMovesBase = function (nonmapView, inputData) {
    ...
    for (var i = 0, lengthi = basemovesbase.length; i < lengthi; i += 1) {
        var _a = basemovesbase[i], departuretime = _a.departuretime, arrivaltime = _a.arrivaltime, operation = _a.operation;
    ...
    if (nonmapView || (depotsBase.length > 0 && typeof bounds !== 'undefined' && Object.keys(bounds).length > 0)) {
        for (var i = 0, lengthi = depotsBase.length; i < lengthi; i += 1) {
            ...
            if (nonmapView ||
                ...
                var itemmap = __assign({ position: [parseFloat(position[0].toString()), parseFloat(position[1].toString()), parseFloat(position[2].toString())] }, getOptionFunction(props, i));
        ...
        if (typeof departuretime !== 'number' || typeof arrivaltime !== 'number') {
            ...
            for (var j = 0, lengthj = operation.length; j < lengthj - 1; j += 1) {
                ...
                if (elapsedtime <= settime && settime < nextelapsedtime) {
                    ...
                    movedData.push(__assign({ position: [position[0] - positionprogress[0], position[1] - positionprogress[1],
