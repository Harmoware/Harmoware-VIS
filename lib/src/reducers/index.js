...
reducer.case(increaseTime, function (state, props) {
    ...
    if ((now - state.starttimestamp) > state.loopTime) {
        ...
        return Object.assign({}, state, {
            settime: settime_1, starttimestamp: starttimestamp, movedData: movedData_1
        });
    }
    ...
    return Object.assign({}, state, {
        settime: settime, beforeFrameTimestamp: beforeFrameTimestamp, movedData: movedData
    });
});
reducer.case(decreaseTime, function (state, props) {
    ...
    var movedData = getMoveObjects(setProps);
    return Object.assign({}, state, {
        settime: settime, starttimestamp: starttimestamp, beforeFrameTimestamp: beforeFrameTimestamp, movedData: movedData
    });
});
...
reducer.case(setFrameTimestamp, function (state, props) {
    ...
    var movedData = getMoveObjects(setProps);
    return Object.assign({}, state, {
        beforeFrameTimestamp: beforeFrameTimestamp, movedData: movedData
    });
});
...
reducer.case(setDepotsBase, function (state, depots) {
    ...
    var depotsData = getDepots(setState);
    return Object.assign({}, state, {
        depotsBase: depotsBase, depotsData: depotsData, depotsBaseOriginal: depotsBaseOriginal
    });
});
...
reducer.case(updateMovesBase, function (state, base) {
    ...
    if (state.movesbase.length === 0 || timeLength === 0) { //初回？
        ...
        return Object.assign({}, state, {
            ...
            depotsBase: depotsBase, depotsData: depotsData, linemapData: linemapData
        });
    }
    var startState = {};
    if (timeLength > 0) {
