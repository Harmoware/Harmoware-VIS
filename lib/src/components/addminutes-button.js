...
const AddMinutesButton = (props) => {
    ...
    const title = React.useMemo(() => propTitle || (children && children.toString()) || `${prop_addMinutes} ${i18n.minutesCaption}`, [children, i18n.minutesCaption]);
    ...
    const caption = React.useMemo(() => children === undefined ? React.createElement(React.Fragment, null, defaultCaption) : React.createElement("span", null, children), [children, i18n.minutesCaption]);
    const func_addMinutes = (minutes) => {
        props.actions.addMinutes(minutes);
    };
    const Result = React.useMemo(() => React.createElement("button", { onClick: () => func_addMinutes(prop_addMinutes), className: className, title: title }, caption), [children, i18n.minutesCaption]);
    return Result;
};
