...
const HarmoVisLayers = (props) => {
    ...
    const { actions, visible, viewport, mapStyle, mapboxApiAccessToken, layers, mapGlComponents, mapboxAddLayerValue, mapboxAddSourceValue, terrain, terrainSource, setTerrain, deckGLProps } = props;
    ...
    const initialize = React.useCallback((gl) => {
    ...
    if (visible) {
        return (React.createElement(MapGl, Object.assign({}, viewport, { mapStyle: mapStyle, onViewportChange: onViewportChange, mapboxApiAccessToken: mapboxApiAccessToken, visible: visible, transitionDuration: transitionDuration, transitionInterruption: transitionInterruption, mapboxAddLayerValue: mapboxAddLayerValue, mapboxAddSourceValue: mapboxAddSourceValue, terrain: terrain, terrainSource: terrainSource, setTerrain: setTerrain }),
            mapGlComponents,
            React.createElement(DeckGL, Object.assign({}, deckGLProps, { viewState: viewport, layers: layers, onWebGLInitialized: initialize }))));
    }
    else {
        const viewState = Object.assign(Object.assign({}, viewport), { transitionDuration, transitionInterruption });
        return (React.createElement(DeckGL, Object.assign({}, deckGLProps, { controller: { type: MapController }, onViewStateChange: (v) => onViewportChange(v.viewState), viewState: viewState, layers: layers, onWebGLInitialized: initialize })));
    }
};
