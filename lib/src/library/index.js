...
const radians = (degree) => degree * RADIANS;
const DEGREES = 180 / pi;
const degrees = (radian) => radian * DEGREES;
...
export const analyzeMovesBase = (state, inputData, update) => {
    ...
    for (let i = 0, lengthi = movesbase.length; i < lengthi; i = (i + 1) | 0) {
        ...
        let sortFlg = false;
        const operation = movesbase[i].operation.reduce((operation, operationElement) => {
            ...
            const movedData = movesbase.reduce((movedData, movesbaseElement, movesbaseidx) => {
                ...
                if (timeLength > 0 && departuretime <= settime && settime < arrivaltime) {
                    ...
                    if (typeof operation[idx].position === 'undefined' ||
                        ...
                        const _b = operation[idx], { elapsedtime, position: sourcePosition, color: sourceColor = settings.COLOR1, direction = 0 } = _b, otherProps2 = __rest(_b, ["elapsedtime", "position", "color", "direction"]);
                        const { elapsedtime: nextelapsedtime, position: targetPosition, color: targetColor = settings.COLOR1 } = operation[nextidx];
...
export const onDefaultClick = (event) => {
...
export const getConfigureStore = (combined) => {
