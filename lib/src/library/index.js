...
export var analyzeMovesBase = function (inputData) {
    ...
    for (var i = 0, lengthi = movesbase.length; i < lengthi; i = (i + 1) | 0) {
        ...
        movesbase[i].operation = operation.reduce(function (operation, operationElement) {
            if (operation.length === 0 || operation.findIndex(function (data) { return data.elapsedtime === operationElement.elapsedtime; }) < 0) {
                operation.push(operationElement);
            }
            return operation;
        }, []);
        for (var j = 0, lengthj = operation.length; j < lengthj; j = (j + 1) | 0) {
            var _c = operation[j], longitude = _c.longitude, latitude = _c.latitude, _d = _c.position, position = _d === void 0 ? [longitude, latitude, 3] : _d;
            if ((typeof operation[j].longitude !== 'number' || typeof operation[j].latitude !== 'number') &&
                typeof operation[j].position === 'undefined') {
                continue;
            }
            if (operation[j].longitude)
                delete operation[j].longitude;
            if (operation[j].latitude)
                delete operation[j].latitude;
            ...
            if (!baseBounds && position[0] && position[1]) {
                var _e = bounds || null, eastlongitiude = _e.eastlongitiude, westlongitiude = _e.westlongitiude, southlatitude = _e.southlatitude, northlatitude = _e.northlatitude;
        ...
        if (typeof baseTimeBegin === 'number') {
            if (!elapsedtimeMode || elapsedtimeMode !== 'UNIXTIME') {
                for (var _i = 0, movesbase_1 = movesbase; _i < movesbase_1.length; _i++) {
                    ...
                    for (var _f = 0, operation_1 = operation; _f < operation_1.length; _f++) {
                        var operationElement = operation_1[_f];
            ...
            else if (typeof baseTimeLength !== 'number') {
                timeLength = safeSubtract(timeEnd, timeBegin);
            }
        }
        else if (typeof baseTimeLength !== 'number') {
