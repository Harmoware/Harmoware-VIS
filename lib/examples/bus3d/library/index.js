...
var getOptionValue = function (optionData) {
    var returnValue = {};
    if (optionData) {
        var elevation = optionData.elevation, color = optionData.color, memo = optionData.memo;
        if (elevation) {
...
export var getBusstopOptionValue = function (props, busstopsbaseidx) {
    var depotsBase = props.depotsBase, settime = props.settime, timeBegin = props.timeBegin, selectedBusstop = props.selectedBusstop, hovered = props.hovered;
    var currentTime = settime - timeBegin;
    ...
    if (Object.keys(bustripindex).length === 0) {
        ...
        bustripscsv.forEach(function (csvdata) {
            ...
            if (timetable.match(/\d{1,2}:\d\d/) && actualdep.match(/\d{1,2}:\d\d:\d\d/) && bssidx_1[busstopcode]) {
                var hms = actualdep.split(':').map(function (current) { return Number(current); });
                var timeDeparture = new Date(date_1[0], date_1[1], date_1[2], hms[0], hms[1], hms[2], 0).getTime() / 1000;
